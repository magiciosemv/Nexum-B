...
async function main() {
  ...
  const { ciphertext: ct } = encrypt(plaintext, kp.publicKey);
  ...
  const { ciphertext: ct_lo } = encrypt(value_lo, kp.publicKey);
  ...
  const { ciphertext: ct1 } = encrypt(1000n, kp.publicKey);
  const { ciphertext: ct2 } = encrypt(2000n, kp.publicKey);
