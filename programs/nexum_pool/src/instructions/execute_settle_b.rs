...
/// Transfer amounts are private — verified inside the ZK circuit.
...
pub fn handler(ctx: Context<ExecuteSettleB>, p: SettleAtomicParams) -> Result<()> {
    ...
    // ── Step 2: Commitment hash verification via ZK proof ─────────────
    // The ZK circuit proves: SHA256(preimage) == commitment_hash
    // where preimage includes the transfer amounts (private inputs).
    // The contract only checks that the commitment_hash from the ZK proof
    // matches the one stored in the CommitSlot.
    let proof_hash_lo = p.commitment_hash_lo;
    let proof_hash_hi = p.commitment_hash_hi;

    // Verify the commitment hash from the proof matches the on-chain slot
    // Split hash into lo/hi limbs using big-endian interpretation
    // (matches TypeScript splitHashToLimbs and ZK circuit public signals)
    let slot_hash_bytes = slot.commitment_hash;
    let expected_hi = u128::from_be_bytes(slot_hash_bytes[0..16].try_into().unwrap());
    let expected_lo = u128::from_be_bytes(slot_hash_bytes[16..32].try_into().unwrap());

    require!(
        proof_hash_lo == expected_lo && proof_hash_hi == expected_hi,
